...
static int imx_pinctrl_set_state(struct udevice *dev, struct udevice *config)
{
	...
	prop = dev_read_prop(config, "fsl,pins", &size);
	if (!prop) {
	...
	if (dev_read_u32_array(config, "fsl,pins", pin_data, size >>2)) {
		      ...
		      struct imx_pinctrl_soc_info *info)
{
	...
	info->mux_mask = dev_read_u32_default(dev, "fsl,mux_mask", 0);
	/*
	 * Refer to linux documentation for details:
	 * Documentation/devicetree/bindings/pinctrl/fsl,imx7d-pinctrl.txt
	 */
	if (dev_read_bool(dev, "fsl,input-sel")) {
		ret = dev_read_phandle_with_args(dev, "fsl,input-sel",
						     NULL, 0, 0, &arg);
		if (ret) {
